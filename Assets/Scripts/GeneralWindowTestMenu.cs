...
public class GeneralWindowTestMenu : MonoBehaviour
{
	...
	private void OnGUI()
	{
		if (!GeneralWindow.IsCreated("Test"))
		{
			...
			num3 += 22;
			num2 -= (num + 4) * 3;
			if (GUI.Button(new Rect((float)num2, (float)num3, (float)num, 24f), "つぶやきCancel Window"))
			{
				GeneralWindow.CInfo info5 = new GeneralWindow.CInfo
				{
					name = "Test",
					buttonType = GeneralWindow.ButtonType.TweetCancel,
					caption = "つぶやきCancelボタンWindow",
					imageName = "リング",
					imageCount = "99999コ"
				};
			...
			num2 += num + 4;
			if (GUI.Button(new Rect((float)num2, (float)num3, (float)num, 24f), "Close Window"))
			{
				GeneralWindow.CInfo info6 = new GeneralWindow.CInfo
				{
					name = "Test",
			...
			num2 += num + 4;
			if (GUI.Button(new Rect((float)num2, (float)num3, (float)num, 24f), "Ok(Next) Window"))
			{
				GeneralWindow.CInfo info7 = new GeneralWindow.CInfo
				{
					name = "Test",
					buttonType = GeneralWindow.ButtonType.Ok,
					caption = "ミッションコンプリート！",
					events = new GeneralWindow.CInfo.Event[]
					{
						new GeneralWindow.CInfo.Event
						{
							faceWindows = new GeneralWindow.CInfo.Event.FaceWindow[]
							{
								new GeneralWindow.CInfo.Event.FaceWindow
								{
									texture = GeneralWindow.GetDummyTexture(0)
								}
							},
							message = "「エッグマンを倒してくれて\n\u3000ありがとう！\n\u3000助けたフリッキーから\n\u3000プレゼントをもらったよ\n\u3000ほんとうに、ありがとう！！」"
						},
						new GeneralWindow.CInfo.Event
						{
							faceWindows = new GeneralWindow.CInfo.Event.FaceWindow[]
							{
								new GeneralWindow.CInfo.Event.FaceWindow
								{
									texture = GeneralWindow.GetDummyTexture(1)
								}
							},
							message = "「おのれ～\u3000ソニックめ\n\u3000こうなったらワシ直々に\n\u3000相手をしてやるわい！」"
						}
				...
				GeneralWindow.Create(info7);
			}
			num2 += num + 4;
			if (GUI.Button(new Rect((float)num2, (float)num3, (float)num, 24f), "Ok(Next) Window2"))
			{
				GeneralWindow.CInfo info8 = new GeneralWindow.CInfo
				{
					name = "Test",
					buttonType = GeneralWindow.ButtonType.Ok,
					caption = "ミッションコンプリート！",
					events = new GeneralWindow.CInfo.Event[]
					{
						...
						new GeneralWindow.CInfo.Event
						{
							...
							message = "「おのれ～\u3000ソニックめ\f\u3000こうなったらワシ直々に\n\u3000相手をしてやるわい！」\f次回、エッグマンを倒せ！"
						}
					}
				};
				GeneralWindow.Create(info8);
			}
		}
		else
		{
			...
			if (GeneralWindow.IsNoButtonPressed)
			{
			}

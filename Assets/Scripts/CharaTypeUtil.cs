...
{
	private static readonly CharaParam[] CHARA_PARAMS = new CharaParam[]
	{
	...
	public static SystemData.CharaTutorialFlagStatus GetCharacterSaveDataFlagStatus(CharaType type)
	{
		...
		{
			return CharaTypeUtil.CHARA_PARAMS[(int)type].m_flagStatus;
		}
	...
	public static HudTutorial.Id GetCharacterTutorialID(CharaType type)
	{
		if (type < CharaType.NUM)
		{
			return CharaTypeUtil.CHARA_PARAMS[(int)type].m_tutorialID;
		}
